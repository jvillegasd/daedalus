...
export default defineBackground(() => {
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => { (async () => {
    ...
    if (message.type === 'toggle-pref') { const p = await prefs(); const list = p[message.field as 'darkDomains' | 'autoplayAllowlist' | 'consentDomains']; const next = list.includes(message.domain) ? list.filter(d => d !== message.domain) : [...list, message.domain]; await chrome.storage.sync.set({ prefs: { ...p, [message.field]: next } }); sendResponse(next.includes(message.domain)); }
