...
async function render() { const list = await groups(); el('groups').innerHTML = list.map(g => `<article><div class="title"><b>${escape(g.name)}</b><span class="count">${g.tabs.length} tabs</span></div>${g.tags.length ? `<div>${g.tags.map(t => `<span class="chip">${escape(t)}</span>`).join(' ')}</div>` : ''}<div class="actions"><button class="btn" data-id="${g.id}">Open</button><button class="btn btn-danger" data-delete="${g.id}">Delete</button></div></article>`).join('') || '<p class="hint">No saved groups yet.</p>'; document.querySelectorAll('[data-id]').forEach(b => b.addEventListener('click', async () => { const g = list.find(x => x.id === (b as HTMLElement).dataset.id)!; for (const t of g.tabs) await chrome.tabs.create({ url: t.url, active: false }); })); document.querySelectorAll('[data-delete]').forEach(b => b.addEventListener('click', async () => { if (confirm('Delete this saved group?')) { await removeGroup((b as HTMLElement).dataset.delete!); render(); } })); }
